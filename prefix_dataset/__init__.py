...
class PrefixDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, num=10, split="train", download=False):
        ...
        self.id2word = pickle.load(open(os.path.join(root, self._suffix, "id_to_word"), "rb"))
        self.word2id = pickle.load(open(os.path.join(root, self._suffix, "word_to_id"), "rb"))
    ...
    def download(self):
        ...
        for url in self.wgets:
            name = url.rsplit("/", 1)[-1]
            download_url(url.format(num=self.num), os.path.join(root, self._suffix), name, self.md5[self.num][name])

    def __repr__(self):
        fmt_str = 'Dataset {}-{}\n'.format(self.__class__.__name__, self.num)
        fmt_str += '    Number of datapoints: {}\n'.format(self.__len__())
        fmt_str += '    Split: {}\n'.format(self.split)
        fmt_str += '    Root/data Location: {}\n'.format(os.path.join(self.root, self._suffix))
        return fmt_str
